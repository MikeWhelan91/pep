...
export function CartDrawer() {
  ...
  const sheet = open ? (
    <div className="fixed inset-0 z-[100]">
      ...
      <aside className="absolute right-0 top-0 flex h-dvh w-full max-w-md translate-x-0 flex-col rounded-l-[2rem] border-l border-cyan-300/20 bg-slate-950 text-slate-100 shadow-[-28px_0_80px_rgba(0,0,0,.55)]">
          ...
          {lines.length === 0 ? (
            <div className="grid min-h-80 place-items-center rounded-3xl border border-cyan-300/15 bg-slate-900/55 p-8 text-center">
              <div>
                ...
                <p className="mt-2 text-sm leading-6 text-slate-400">Add catalogue items to review quantity, availability, and checkout totals here.</p>
            ...
            <div className="grid gap-3">
              {lines.map((line) => (
                <div key={line.productId} className="rounded-3xl border border-cyan-300/15 bg-slate-900/60 p-4">
                  ...
                  <p className="mt-1 text-xs text-slate-500">{line.product.stockQuantity} available</p>
